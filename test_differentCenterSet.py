...
validation_dir = main_dir + '/WollishofenData'
data_csv = validation_dir + '/dataset_information/AllSubject_UUIDs.csv'
...
np.savetxt(os.path.join(model_dir, model_name+'_predictions_WollishofenData.txt'), test_predictions)
test_predictions = np.asarray(test_predictions)
prob_predictions = 1 / (1 + np.exp(-test_predictions))
np.savetxt(os.path.join(model_dir, model_name+'_prob_predictions_WollishofenData.txt'), prob_predictions)
np.savetxt(os.path.join(model_dir, model_name+'_labels_WollishofenData.txt'), test_gt_labels)
